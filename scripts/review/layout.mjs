...
export function reviewShell(entry,release) {
  ...
  const links=(mobile=false)=>REVIEW_PAGES.map(p=>`<a href="${reviewHref(p.slug,entry.depth)}" data-review-nav${p.slug===entry.slug?' aria-current="page"':''}><span class="review-nav__n">${p.number}</span><span><b>${p.title}</b>${mobile?'':`<small>${p.blurb}</small>`}</span></a>`).join('');
  ...
  const header=`<header class="reviewhead"><div class="reviewhead__in">
    ...
    <div class="reviewhead__status cap"><span>${REVIEW.edition}</span><span data-review-version>${status}</span><span>${money(REVIEW.request)} proposed</span></div>
  ...
  const rail=`<aside class="review__rail"><p class="cap">Review / ${REVIEW.edition}</p><nav class="review-nav" aria-label="Review room">${links()}</nav><div class="review__rail-end"><a href="${SITE.repo}">Source ↗</a><a href="${reviewHref('try',entry.depth)}">Download ↗</a><span data-review-version>${status}</span></div></aside>`;
