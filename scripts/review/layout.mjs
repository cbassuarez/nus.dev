import {SITE, icon, nusMark, rel} from '../layout.mjs';
...
export function reviewShell(entry,release) {
  ...
  const header=`<header class="reviewhead"><div class="reviewhead__in">
    ...
    <div class="controls"><div class="signalpick" data-signalpick role="group" aria-label="Signal colour"></div><button class="iconbtn" type="button" data-theme-toggle aria-label="Switch theme"><span data-sun hidden>${icon('sun')}</span><span data-moon>${icon('moon')}</span></button></div>
  ...
  const pager=`<nav class="review-pager" aria-label="Adjacent review pages">${adjacent.map((p,i)=>p?`<a href="${reviewHref(p.slug,entry.depth)}"><span class="cap">${i?'Next':'Previous'}</span><span>${i?'':'← '}${p.title}${i?' →':''}</span></a>`:'<span></span>').join('')}</nav>`;
