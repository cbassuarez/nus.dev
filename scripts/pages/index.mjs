...
<section class="section">
  <div class="section__in">
    ...
    <p class="lede" style="margin-bottom:26px">
      The palette, ask, live folders, the atlas, stacks, the site panel, tiles, peek,
      picture-in-picture, compact. Each drawn from the app's own tokens, each linking
      the module that builds it.
    </p>
    <div class="teaser">
      ${[
        ['command', 'The palette', '⌘K'],
        ['sparkle', 'Ask', '⌘⇧?'],
        ['plugs-connected', 'Live folders', 'Ports · GitHub'],
        ['planet', 'The atlas', 'Restore'],
        ['stack', 'Stacks', 'rules.luau'],
        ['shield-check', 'The site panel', 'Per host'],
        ['squares-four', 'Tiles', 'Ctrl+⇧+D'],
        ['corners-out', 'Peek', 'Alt+click'],
        ['picture-in-picture', 'Picture-in-picture', 'Ours'],
        ['sidebar-simple', 'Compact', 'Ctrl+⇧+B']
      ].map(([i, n, note]) => `<a class="teaser__i" href="./panels/">
        ${icon(i)}<b>${n}</b><span>${note}</span>
      </a>`).join('')}
    </div>
    <p style="margin-top:24px"><a class="btn btn--fill" href="./panels/">${icon('squares-four')}See the panels</a></p>
  </div>
</section>
